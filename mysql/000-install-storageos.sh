...
ssh  -o "StrictHostKeyChecking=no" root@"[[HOST_IP]]" "docker run --name enable_lio \
--privileged --rm --cap-add=SYS_ADMIN -v /lib/modules:/lib/modules            \
-v /sys:/sys:rshared storageos/init:0.2;                                       \
docker -H [[HOST_IP]]:2345 run -d --name storageos                                 \
-e DISABLE_TELEMETRY=true -e HOSTNAME=node02                                   \
-e ADVERTISE_IP=[[HOST_IP]] -e JOIN=[[HOST_IP]] --net=host --pid=host            \
--privileged --cap-add SYS_ADMIN --device /dev/fuse -v /sys:/sys                   \
